...
export async function atualizarFuncionario(id,dados){if(!id)throw new Error("Identificador do funcionário não informado.");const b=await banco();const atual=await obterFuncionario(id);if(!atual)throw new Error("Funcionário não encontrado.");const setor=normalizarSetor(dados);await updateDoc(doc(b,COLLECTION,id),{...dados,setorTrabalho:setor,setorId:setor,atualizadoEm:new Date().toISOString()});}
